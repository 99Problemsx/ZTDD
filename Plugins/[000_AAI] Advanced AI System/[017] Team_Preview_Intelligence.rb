...
#===============================================================================
# Wählt optimalen Lead basierend auf Gegner-Team (Team Preview)
#
# Features:
# - Lead Role Detection (Hazards, Fast Attacker, Weather Setter, etc.)
# - Matchup Analysis (welcher Lead ist am besten?)
# - Anti-Lead Counter (Gegner hat Fake Out? → Ghost Lead)
# - Team Synergy (Lead passt zu Rest-Team?)
#===============================================================================
...
module AdvancedAI
  module TeamPreviewIntelligence
    ...
    LEAD_ROLES = {
      :hazard_lead => {
        description: "Setzt Stealth Rock/Spikes",
        priority: 80,
        requires: [:hazard_moves, :decent_speed_or_focus_sash]
      },
      
      :weather_lead => {
        description: "Setzt Weather für Team",
        priority: 85,
        requires: [:weather_ability_or_move, :team_benefits_from_weather]
      },
      
      :terrain_lead => {
        description: "Setzt Terrain für Team",
        priority: 85,
        requires: [:terrain_ability_or_move, :team_benefits_from_terrain]
      },
      
      :fast_attacker => {
        description: "Schneller Angreifer für frühen Druck",
        priority: 70,
        requires: [:high_speed, :high_attack_or_spatk]
      },
      
      :fake_out_lead => {
        description: "Fake Out für Momentum",
        priority: 75,
        requires: [:fake_out_move]
      },
      
      :anti_lead => {
        description: "Countered typische Leads (Taunt, etc.)",
        priority: 75,
        requires: [:taunt_or_fast_attack, :high_speed]
      },
      
      :screen_setter => {
        description: "Setzt Screens für Setup-Sweeper",
        priority: 70,
        requires: [:screen_moves, :team_has_setup_sweepers]
      },
      
      :suicide_lead => {
        description: "Explosion/Hazards dann stirbt",
        priority: 60,
    ...
    def self.evaluate_lead(battle, pokemon, party, opponent_party)
      ...
      if item
        ...
        score -= 10 if item == :CHOICEBAND     # Locked = riskant als Lead
        score -= 10 if item == :CHOICESPECS
      end
      
      # 5. Hazard Immunity
      if pokemon.hasType?(:FLYING) || pokemon.ability == :LEVITATE
        score += 8  # Immune zu Stealth Rock/Spikes teilweise
      end
      
      if pokemon.item_id == :HEAVYDUTYBOOTS
        score += 12  # Vollständige Hazard-Immunity
      end
    ...
    #===========================================================================
    # Lead Role Detection für Pokemon
    #===========================================================================
    ...
    # Analysiert Matchups gegen Gegner-Team
    def self.analyze_matchups(pokemon, opponent_party)
      ...
      # Bonus wenn gegen viele favorable
      score += 20 if favorable_matchups >= 3
      
      # Penalty wenn gegen viele unfavorable
      score -= 15 if unfavorable_matchups >= 3
      
      return score
    end
    
    # Berechnet Type-Matchup Score
    ...
    # Countered typische Hazard-Leads
    def self.counter_hazard_lead(pokemon, opponent_party)
      return 0 if !pokemon || !opponent_party
      
      # Prüfe ob Gegner wahrscheinlich Hazard-Lead hat
