...
# Strategisches Tera-Timing und Typ-Vorteil-Analyse
#===============================================================================

class Battle::AI
  # Prüft ob Terastallize genutzt werden sollte
  def should_terastallize?(user, skill)
    ...
    # Schwellenwerte
    return true if score >= 80   # Notfall/garantierter Wert
    return true if score >= 60   # Starke Situation
    return true if score >= 40 && remaining_pokemon_count(user) <= 2  # Gut + wenig übrig
  ...
  def evaluate_tera_timing(user, skill)
    ...
    if turn <= 2
      # Early Game: Nur bei starkem Vorteil
      score += 10
    elsif turn <= 6
      # Mid Game: Optimal für Sweep-Setup
      score += 20
    else
      # Late Game: Cleanup oder Notfall
      score += 15
    end
  ...
  def evaluate_tera_type_advantage(user, skill)
    ...
    user.moves.each do |move|
      ...
      if move.type == tera_type
        # STAB Bonus nach Tera
        score += 15
        
        # Coverage gegen aktive Gegner
  ...
  def evaluate_tera_survival(user, skill)
    ...
    # Überlebens-Priorität
    if remaining_pokemon_count(user) == 1
      score += 20  # Letztes Pokemon
    end
  ...
  def evaluate_tera_party(user, skill)
    ...
    better_candidates = party.count do |pkmn|
      ...
      # Höhere Offensive oder bessere Tera-Synergie
...
# Erweiterte Battler-Methoden für Terastallization
