...
module AdvancedAI
  module FieldEffects
    # Hauptfunktion: Berechnet Field Effect Bonus
    ...
    def self.weather_bonus(battle, move, user, target)
      ...
      when :Hail, :Snow
        ...
        bonus += 20 if move.id == :WEATHERBALL
        bonus += 25 if move.id == :AURORAVEIL  # Aurora Veil nur bei Hail/Snow
    ...
    def self.trick_room_bonus(battle, move, user, target)
      ...
      # Wenn Trick Room aktiv, bevorzuge langsame Pokemon
      if user.pbSpeed < 50
        bonus += 20  # Slow Pokemon profitiert
      elsif user.pbSpeed > 120
        bonus -= 20  # Fast Pokemon benachteiligt
      end
      
      # Priority Moves sind stärker in Trick Room
      bonus += 15 if move.priority > 0
    ...
    def self.room_bonus(battle, move, user, target)
      ...
      if battle.field.effects[PBEffects::MagicRoom] > 0
        # Item-abhängige Moves sind schwächer
    ...
    def self.weather_setting_bonus(battle, move, user, skill_level = 100)
      ...
      # Prüfe ob Team von Weather profitiert
