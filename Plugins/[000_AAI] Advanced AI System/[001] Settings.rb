...
module AdvancedAI
  ...
  SKILL_THRESHOLDS = {
    ...
    :team_preview      => 90,   # Team Preview Intelligence
  ...
  # Get AI tier based on skill level
  def self.get_ai_tier(skill_level)
    return :basic if skill_level < SKILL_THRESHOLDS[:core]
    return :core if skill_level < SKILL_THRESHOLDS[:setup]
    return :advanced if skill_level < SKILL_THRESHOLDS[:items]
    return :expert if skill_level < SKILL_THRESHOLDS[:team_preview]
    return :master
  end
...
if defined?(Settings::CHALLENGE_MODE)
  Events.onStartOfBattle += proc { |_sender, battle|
