...
# Erkennt 7 Pokemon-Rollen: Sweeper, Wall, Tank, Support, Wallbreaker, Pivot, Lead
#===============================================================================
...
module AdvancedAI
  module RoleDetection
    ...
    # Erkennt primäre und sekundäre Rolle
    ...
    def self.recommend_role_for_situation(battle, fainted_index, opponent, skill_level = 100)
      ...
      # Analyse Gegner-Rolle
      opp_role, _ = detect_roles(opponent)
      
      # Counter-Pick
      case opp_role
      when :sweeper
        return :wall  # Wall stoppt Sweeper
      when :wall
        return :wallbreaker  # Wallbreaker bricht Wall
      when :wallbreaker
        return :sweeper  # Outspeede Wallbreaker
      when :support
...
class Battle::AI
  def evaluate_switch_with_roles(user, opponent, skill_level)
    ...
    # Finde Pokemon mit empfohlener Rolle
