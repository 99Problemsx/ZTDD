...
EventHandlers.add(:on_pokemon_received, :monotype_check,
  proc { |pkmn, method|
    ...
    next if !pkmn
    next if ChallengeModes.valid_monotype_pokemon?(pkmn)
    ...
    next false  # Cancel the Pokemon reception
  }
)

# Additional check for gift Pokemon through pbAddPokemon
module Kernel
  alias __monotype__pbAddPokemon pbAddPokemon
  def pbAddPokemon(pokemon, level = 1, see_form = true)
    if ChallengeModes.on?(:MONOTYPE_MODE) && pokemon.is_a?(Pokemon)
      if !ChallengeModes.valid_monotype_pokemon?(pokemon)
        type_name = GameData::Type.get($PokemonGlobal.challenge_monotype_type).name
    ...
    return __monotype__pbAddPokemon(pokemon, level, see_form)
  end
end
