...
# Fix für Pokemon Position nach dem Kampf
EventHandlers.add(:on_end_battle, :following_pokemon_battle_end, proc { |_decision, _canLose|
  # Refresh following Pokemon sprite after battle
  FollowingPkmn.refresh(false) if FollowingPkmn.can_check?
  
  # Pokemon hinter den Spieler positionieren statt auf ihn
  if $game_temp.followers
    leader = $game_player
    $game_temp.followers.each_follower do |event, follower|
      # Berechne Position hinter dem Spieler basierend auf Blickrichtung
      case leader.direction
      when 2  # Down
        event.moveto(leader.x, leader.y - 1)
      when 4  # Left  
        event.moveto(leader.x + 1, leader.y)
      when 6  # Right
        event.moveto(leader.x - 1, leader.y)
      when 8  # Up
        event.moveto(leader.x, leader.y + 1)
