...
module NightTilesetHelper
  ...
  def self.get_night_variant(filename)
    ...
    # Debug output disabled - too spammy in battle
...
def pbGetTileset(name, hue = 0)
  ...
  if PBDayNight.isNight? && NightTilesetHelper.night_tileset_enabled?
    ...
    # Debug output only when not in battle
    # echoln("[Night Tilesets] Checking for night tileset: Graphics/Tilesets/#{night_name}") if $DEBUG && !$game_temp&.in_battle
    
    if pbResolveBitmap("Graphics/Tilesets/" + night_name)
      # echoln("[Night Tilesets] SUCCESS! Loading night tileset: #{night_name}") if !$game_temp&.in_battle
      return AnimatedBitmap.new("Graphics/Tilesets/" + night_name, hue).deanimate
    else
      # echoln("[Night Tilesets] Not found, using day version: #{name}") if $DEBUG && !$game_temp&.in_battle
    end
  end
...
def pbGetAutotile(name, hue = 0)
  ...
  if PBDayNight.isNight? && NightTilesetHelper.night_tileset_enabled?
    ...
    # echoln("[Night Tilesets] Checking for night autotile: Graphics/Autotiles/#{night_name}") if $DEBUG && !$game_temp&.in_battle
    
    if pbResolveBitmap("Graphics/Autotiles/" + night_name)
      # echoln("[Night Tilesets] SUCCESS! Loading night autotile: #{night_name}") if !$game_temp&.in_battle
